...
class WaterQualityPredictor:
    def __init__(self, model_file, scaler_file, use_parallel=True, max_workers=None):
        ...
        if use_parallel:
            ...
            # Load models directly for sequential processing
    ...
    def _process_chunk_parallel(self, chunk_info: Dict[str, Any]) -> Tuple[int, int, np.ndarray]:
        ...
        try:
            result = self._calculate_chunk_predictions(bands_chunk, month, season, model, scaler)
    ...
    def _process_chunk_sequential(self, bands_chunk, month, season):
        ...
        return self._calculate_chunk_predictions(bands_chunk, month, season, self.model, self.scaler)
                ...
                for y in range(0, height, chunk_size):
                    ...
                    for x in range(0, width, chunk_size):
                        ...
                        if chunk_count % 10 == 0:  # Progress update every 10 chunks
                            ...
                            print(f"Processing chunk {chunk_count}/{total_chunks} ({progress:.1f}%)")

...
class MapGenerator:
    def __init__(self, raster_path, image_date):
    ...
    def create_interactive_map(self):
        ...
        with rasterio.open(self.raster_path) as src:
            ...
            folium.TileLayer(
                "cartodbpositron", name="CartoDB Positron"
            ).add_to(m)
            folium.TileLayer(
                "cartodbdark_matter", name="CartoDB Dark Matter"
            ).add_to(m)

            # Try to add Sentinel-2 imagery
            try:
                start_date = self.image_date.strftime("%Y-%m-%d")
                end_date = (self.image_date + timedelta(days=1)).strftime(
                    "%Y-%m-%d"
                )
                ...
                if s2_image:
                    ...
                    print(
                        f"Successfully added Sentinel-2 mosaic for {start_date}"  # noqa
                    )
                else:
            ...
            norm = mcolors.Normalize(
                vmin=np.min(valid_data), vmax=np.max(valid_data)
            )
